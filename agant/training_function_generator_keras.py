...
def training_fucntion_generator(conf_data):
    ...
    if seq == 0:
        ...
        if classes > 0:
            sampled_labels = np.random.randint(0, classes, mini_batch_size).reshape(-1, 1)
            g_loss = combined_model.train_on_batch([z, sampled_labels], valid)
            conf_data['g_loss'] = g_loss
        elif classes <= 0:
            if w_loss == 1:
                g_loss = - combined_model.train_on_batch(z,valid)
            elif w_loss == 0:
                g_loss = combined_model.train_on_batch(z,valid)
            conf_data['g_loss'] = g_loss
    ...
    if seq == 0:
        print ("[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f]" % (epoch, epochs, conf_data['iterator'], len(conf_data['data_learn']), 
                                                       conf_data['d_loss'][0], g_loss))
    # print ("Yay atleast one done!")    
    # exit()
    # This is for SeqGAN
    # elif seq == 1:
